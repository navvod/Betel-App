...
MODEL_PATH = os.path.join(BASE_DIR, "api", "ml", "commercial.tflite")
...
def load_model():
    ...
    if interpreter is None:
        try:
            ...
            print("✅ Commercial TFLite model loaded successfully")
            print(f"   Input  dtype : {input_details[0]['dtype']}")
            print(f"   Input  shape : {input_details[0]['shape']}")
        except Exception as e:
            print(f"❌ Error loading commercial TFLite model: {e}")
            interpreter = None
...
def predict_commercial(image_path):
    load_model()
    if interpreter is None:
        raise Exception("Commercial TFLite model not loaded")
