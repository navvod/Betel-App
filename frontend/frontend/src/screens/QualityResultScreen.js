...
export default function QualityResultScreen({ route, navigation }) {
  const { image, quality, confidence } = route.params || {};
  ...
  const getQualityColor = () => {
    ...
    if (quality?.includes("Standard")) return "#F1C40F"; // Yellow
    if (quality?.includes("Poor")) return "#E74C3C";     // Red
    return "#95A5A6"; // Grey
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          ...
          <Text style={[styles.percent, { color: getQualityColor() }]}>{confidencePercent}%</Text>
